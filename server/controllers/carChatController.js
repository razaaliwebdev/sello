...
export const getSellerBuyerChats = async (req, res) => {
  try {
    ...
    // Find all cars posted by this seller (check if user actually has cars)
    ...
    // Find all chats for these cars where the seller is a participant
    const chats = await Chat.find({
      chatType: "car",
      car: { $in: carIds },
      participants: { $in: [req.user._id] },
...
export const sendCarChatMessage = async (req, res) => {
  try {
    ...
    const car = await Car.findById(chat.car).populate(
      "postedBy",
      "name email role"
    );
    if (car && car.isSold) {
    ...
    // Send notification to seller if message is from buyer (not seller themselves)
    if (seller && seller._id.toString() !== buyer._id.toString()) {
      try {
        ...
        await Notification.create({
          ...
          message: `${buyer.name} sent you a message about "${
            car?.title || "your listing"
          }"`,
          type: "info",
